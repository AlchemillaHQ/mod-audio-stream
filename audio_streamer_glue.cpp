...
public:
    ...
    void rawAudioActive(bool active) {
        m_rawAudioActive = active;
    }

    void setRawSampleRate(int rate) { m_rawSampleRate = rate; }
...
private:
    ...
    AudioStreamer(
        ...
        m_rawSampleRate(8000), m_rawAudioActive(false) {
            ...
            if (headers_json) {
                ...
                while (iterator) {
                    if (iterator->type == cJSON_String && iterator->valuestring != nullptr) {
                        hdrs[iterator->string] = iterator->valuestring;
                    }
    ...
    void eventCallback(notifyEvent_t event, const char* message) {
        ...
        switch (event) {
            ...
            case MESSAGE:
                if (pr.ok == SWITCH_TRUE) {
                    if (pr.audioDataType == "raw" && pr.decodedAudio.empty()) {
                        // rawAudio metadata - configure binary stream
                        m_rawAudioActive = true;
                        m_rawSampleRate = pr.sampleRate;
                        m_notify(psession, EVENT_JSON, msg.c_str());
                    } else if (!pr.decodedAudio.empty()) {
                        switch_channel_t *ch = switch_core_session_get_channel(psession);
                        auto *media_bug = (switch_media_bug_t*) switch_channel_get_private(ch, MY_BUG_NAME);
                        if (media_bug) {
                            auto *tp = (private_t*) switch_core_media_bug_get_user_data(media_bug);
                            if (tp && tp->playback_enabled && tp->playback_buffer) {
                                switch_mutex_lock(tp->mutex);

                                const uint8_t *src = pr.decodedAudio.data();
                                size_t src_len = pr.decodedAudio.size();
                                ...
                                if (pr.sampleRate != tp->native_sampling && pr.sampleRate > 0 && src_len > 0) {
                                    ...
                                    if (tp->playback_resampler) {
                                        ...
                                        if (out_len > 0) {
                                            size_t bytes = out_len * 2;
                                            resampled.assign((uint8_t*)out.data(), (uint8_t*)out.data() + bytes);
                                            src = resampled.data();
                                            src_len = bytes;
                                        }
                                    }
    ...
    void onBinaryData(const void* data, size_t len) {
        if (!m_rawAudioActive || !data || len == 0) return;
    ...
    ProcessResult processMessage(const std::string& message) {
        ...
        if (std::strcmp(jsonType, "rawAudio") == 0) {
            cJSON* jd = cJSON_GetObjectItem(root.get(), "data");
            if (jd) {
                out.sampleRate = cJSON_GetObjectItem(jd, "sampleRate") 
                    ? cJSON_GetObjectItem(jd, "sampleRate")->valueint : 8000;
            }
            out.audioDataType = "raw";
            out.ok = SWITCH_TRUE;
            return out;
        }
        ...
        bool isRaw   = (std::strcmp(jsAudioDataType, "raw") == 0);
        bool isPcmu  = (std::strcmp(jsAudioDataType, "pcmu") == 0);
        bool isPcma  = (std::strcmp(jsAudioDataType, "pcma") == 0);
        bool isWav   = (std::strcmp(jsAudioDataType, "wav") == 0);
        bool isMp3   = (std::strcmp(jsAudioDataType, "mp3") == 0);
        bool isOgg   = (std::strcmp(jsAudioDataType, "ogg") == 0);
        bool isOpus  = (std::strcmp(jsAudioDataType, "opus") == 0);

        if (!isRaw && !isPcmu && !isPcma && !isWav && !isMp3 && !isOgg && !isOpus) {
            push_err(out, m_sessionId, "processMessage - unsupported audio type: " + std::string(jsAudioDataType));
            return out;
        }

        if (isRaw && (sampleRate % 8000 != 0 || sampleRate < 8000 || sampleRate > 64000)) {
        ...
        // decode to PCM for playback
        if (isRaw || isPcmu || isPcma) {
            out.decodedAudio.assign(
                reinterpret_cast<const uint8_t*>(decoded.data()),
                reinterpret_cast<const uint8_t*>(decoded.data()) + decoded.size()
            );
            out.sampleRate = isRaw ? sampleRate : 8000;
            out.audioDataType = jsAudioDataType;
        } else if (isOpus) {
            int opus_err;
            OpusDecoder *dec = opus_decoder_create(48000, 1, &opus_err);
            if (dec && opus_err == OPUS_OK) {
                opus_int16 pcm_buf[5760];
                int frames = opus_decode(dec,
                    reinterpret_cast<const unsigned char*>(decoded.data()),
                    static_cast<opus_int32>(decoded.size()),
                    pcm_buf, 5760, 0);
                if (frames > 0) {
                    int bytes = frames * 2;
                    out.decodedAudio.assign(
                        reinterpret_cast<const uint8_t*>(pcm_buf),
                        reinterpret_cast<const uint8_t*>(pcm_buf) + bytes);
                }
        ...
        // build response JSON (audioData already detached, so it's excluded)
...
namespace {
    ...
    switch_status_t stream_data_init(private_t *tech_pvt, switch_core_session_t *session, char *wsUri,
                                     ...
                                     bool tls_disable_hostname_validation, bool playback_enabled)
    {
        int err; //speex
        ...
        // STREAM_SAMPLE_RATE channel var as default for raw binary mode
        const char* rawSampleRateVar = switch_channel_get_variable(
            switch_core_session_get_channel(session), "STREAM_SAMPLE_RATE");
        if (rawSampleRateVar) {
            int rate = atoi(rawSampleRateVar);
            if (rate >= 8000 && rate % 8000 == 0) sp->setRawSampleRate(rate);
        }
...
extern "C" {
                                        ...
                                        void **ppUserData)
    {
        ...
        bool playback_enabled = switch_channel_var_true(channel, "STREAM_PLAYBACK");
    ...
    switch_status_t stream_session_destroy(switch_core_session_t *session, private_t **ppUserData) {
        if (!ppUserData || !*ppUserData) return SWITCH_STATUS_FALSE;
        private_t *tech_pvt = *ppUserData;
        *ppUserData = nullptr;

        std::shared_ptr<AudioStreamer>* sp_wrap = static_cast<std::shared_ptr<AudioStreamer>*>(tech_pvt->pAudioStreamer);
    ...
    switch_status_t stream_session_cleanup(switch_core_session_t *session, char* text, int channelIsClosing) {
        ...
        if(bug)
        {
            auto* tech_pvt = (private_t*) switch_core_media_bug_get_user_data(bug);
            if (!tech_pvt) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "stream_session_cleanup: no user data\n");
                return SWITCH_STATUS_FALSE;
            }
    ...
    static void fire_playback_event(switch_core_session_t *session, private_t *tech_pvt, 
                                     const char *event_type, int seq, int size, int remaining, int total) {
        cJSON *root = cJSON_CreateObject();
        cJSON_AddStringToObject(root, "event", event_type);
        if (seq >= 0) cJSON_AddNumberToObject(root, "seq", seq);
        if (size >= 0) cJSON_AddNumberToObject(root, "size", size);
        if (remaining >= 0) cJSON_AddNumberToObject(root, "remaining", remaining);
        if (total >= 0) cJSON_AddNumberToObject(root, "total_chunks", total);
        char *json_str = cJSON_PrintUnformatted(root);
        
        tech_pvt->responseHandler(session, EVENT_PLAYBACK, json_str);
    ...
    switch_bool_t stream_frame_write(switch_media_bug_t *bug) {
        ...
        if (switch_buffer_inuse(tech_pvt->playback_buffer) >= frame_bytes) {
            ...
            switch_size_t read = (switch_size_t)frame_bytes;
            if (read > sizeof(frame_buf)) read = sizeof(frame_buf);
            ...
            if (replaced) {
                tech_pvt->playback_seq++;
                tech_pvt->playback_chunks_remaining--;
                switch_core_session_t *session = switch_core_media_bug_get_session(bug);
                fire_playback_event(session, tech_pvt, "chunk_played",
                    tech_pvt->playback_seq, (int)read,
                    tech_pvt->playback_chunks_remaining, -1);

                if (tech_pvt->playback_chunks_remaining <= 0) {
                    tech_pvt->playback_in_response = 0;
                    fire_playback_event(session, tech_pvt, "queue_completed",
                        -1, -1, -1, tech_pvt->playback_seq);
                    tech_pvt->playback_seq = 0;
                }
            }
        } else {
    ...
    switch_status_t stream_session_break(switch_core_session_t *session) {
        ...
        auto *bug = (switch_media_bug_t*) switch_channel_get_private(channel, MY_BUG_NAME);
        if (!bug) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "stream_session_break failed because no bug\n");
            return SWITCH_STATUS_FALSE;
        }
        auto *tech_pvt = (private_t*) switch_core_media_bug_get_user_data(bug);
        if (!tech_pvt) return SWITCH_STATUS_FALSE;
        ...
        if (tech_pvt->playback_resampler) {
            speex_resampler_reset_mem(tech_pvt->playback_resampler);
        }
        tech_pvt->playback_in_response = 0;
        tech_pvt->playback_seq = 0;
        tech_pvt->playback_chunks_remaining = 0;
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "playback queue broken\n");
        return SWITCH_STATUS_SUCCESS;
    }
}