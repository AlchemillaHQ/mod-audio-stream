                                     ...
                                     char* metadata)
{
    ...
    if ((status = switch_core_media_bug_add(session, MY_BUG_NAME, NULL, capture_callback, pUserData, 0, flags, &bug)) != SWITCH_STATUS_SUCCESS) {
        stream_session_destroy(session, (private_t**)&pUserData);
        return status;
    }
...
{
    switch_status_t status = SWITCH_STATUS_SUCCESS;
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_audio_stream: break\n");
    status = stream_session_break(session);
    return status;
}
...
#define STREAM_API_SYNTAX "<uuid> [start | stop | send_text | pause | resume | break ] [wss-url | path] [mono | mixed | stereo] [8000 | 16000] [metadata]"
    ...
    if (zstr(cmd) || argc < 2 || (0 == strcmp(argv[1], "start") && argc < 4)) {
        ...
        if ((lsession = switch_core_session_locate(argv[0]))) {
            if (!strcasecmp(argv[1], "stop")) {
                if(argc > 2 && !is_valid_utf8(argv[2])) {
                ...
                if(metadata && !is_valid_utf8(metadata)) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                      "%s contains invalid utf8 characters\n", metadata);
                    switch_core_session_rwunlock(lsession);
                    goto done;
...
SWITCH_STANDARD_APP(uuid_audio_stream_app)
{
    const char *uuid = switch_core_session_get_uuid(session);
    char *full_cmd = switch_mprintf("%s %s", uuid, data);
    switch_stream_handle_t s = { 0 };
    SWITCH_STANDARD_STREAM(s);
    switch_api_execute("uuid_audio_stream", full_cmd, session, &s);
    switch_safe_free(s.data);
    switch_safe_free(full_cmd);
}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_audio_stream_load)
{
    ...
    *module_interface = switch_loadable_module_create_module_interface(pool, modname);
    ...
    if (switch_event_reserve_subclass(EVENT_JSON) != SWITCH_STATUS_SUCCESS ||
        ...
        switch_event_reserve_subclass(EVENT_PLAYBACK) != SWITCH_STATUS_SUCCESS) {
    ...
    switch_console_set_complete("add uuid_audio_stream ::console::list_uuid break");

    switch_application_interface_t *app_interface;
    SWITCH_ADD_APP(app_interface, "uuid_audio_stream", "audio_stream APP", "audio_stream APP",
                   uuid_audio_stream_app, "<start|stop|pause|resume|break|send_text>", SAF_NONE);
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_audio_stream_shutdown)
{
    ...
    switch_event_free_subclass(EVENT_PLAYBACK);
